                                       ...
                                       IsoparametricTransformation *ElTr)
{
   ...
   else
   {
      ...
      else    // L2 Nodes (e.g., periodic mesh)
      {
         ...
         GetLocalFaceTransformation(GetBdrElementType(i), GetElementType(elem_id),
                                    FaceElemTr.Loc1.Transf, face_info);
         // NOTE: FaceElemTr.Loc1 is overwritten here -- used as a temporary

         const FiniteElement *face_el = Nodes->FESpace()->GetTraceElement(
                                           elem_id, GetBdrElementBaseGeometry(i));
...
#ifdef MFEM_USE_GECKO
void Mesh::GetGeckoElementReordering(Array<int> &ordering, int iterations,
                                     int window, int period, int seed)
{
   Gecko::Graph graph;
...
void Mesh::Make3D(int nx, int ny, int nz, Element::Type type, double sx,
                  double sy, double sz, bool sfc_ordering)
{
   int x, y, z;
...
void Mesh::Make2D(int nx, int ny, Element::Type type, double sx, double sy,
...
int *Mesh::GeneratePartitioning(int nparts, int part_method)
{
   ...
   else
   {
      ...
      {
#ifndef MFEM_USE_METIS_5
         METIS_PartGraphRecursive(&n, I, J, NULL, NULL, &wgtflag, &numflag,
                                  &mparts, options, &edgecut, mpartitioning);
#else
         err = METIS_PartGraphRecursive(&n, &ncon, I, J, NULL, NULL, NULL, &mparts,
                                        NULL, NULL, options, &edgecut,
                                        mpartitioning);
         if (err != 1)
            mfem_error(
      ...
      {
#ifndef MFEM_USE_METIS_5
         METIS_PartGraphKway(&n, I, J, NULL, NULL, &wgtflag, &numflag, &mparts,
                             options, &edgecut, mpartitioning);
#else
         err = METIS_PartGraphKway(&n, &ncon, I, J, NULL, NULL, NULL, &mparts,
                                   NULL, NULL, options, &edgecut, mpartitioning);
         if (err != 1)
            mfem_error(
               "Mesh::GeneratePartitioning: "
      ...
      {
#ifndef MFEM_USE_METIS_5
         METIS_PartGraphVKway(&n, I, J, NULL, NULL, &wgtflag, &numflag, &mparts,
                              options, &edgecut, mpartitioning);
#else
                                   ...
                                   int flags)
{
   ...
   const Operator *elem_restr =
      fespace->GetElementRestriction(ElementDofOrdering::NATIVE);
